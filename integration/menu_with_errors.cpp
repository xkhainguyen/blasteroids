...
int GameMenu::RunOneStep()
{
    ...
    auto deltaT=std::chrono::high_resolution_clock::now()-t0;
    if (players_dd.closed_message.empty() && BMENU_Start==val || diff_dd.closed_message.empty() && BMENU_Start==val)
    {
        display_error=true;
        val=BMENU_None;
    }
    if (true==display_error && error_time>=0)
    {
        glColor3f(255,0,0);
        glRasterPos2d(50,400);
        YsGlDrawFontBitmap12x16("you forgot to select on of the option, please make a selection");
        auto millisec=std::chrono::duration_cast<std::chrono::microseconds>(deltaT).count();
        error_time-=millisec;
    }
    else if (true==display_error && error_time<0)
    {
        display_error=false;
        error_time=250000;
    }
